...
export const editDoc = (id) => (dispatch) => {
    ...
    return axios.delete(`http://localhost:8080/doctors/${id}`)
    .then((res)=>{
...
export const delData = (id,status) => (dispatch) => {
    ...
    return axios.patch(`http://localhost:8080/doctors/${id}`,{status:!status})
    .then((resp)=>{
...
export const addDoct = (obj) => (dispatch) => {
    dispatch({type:GET_DOCTOR_REQ})
return axios.post("http://localhost:8080/doctors",obj)
.then((res)=>{
    //console.log(res)
